...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        // Camera params
        bool CameraSwitched = false;    // Is true when near Cam should be used and false if wide cam should be used
        bool CameraSwitchable = true;
        ...
        void FixedUpdate()
        {
            ...
            // Check for Cam change
            checkCameraSwitch(Input.Jump);

            // apply vehicle physics
            if (m_CanMove)
            {
                MoveVehicle(Input.Accelerate, Input.Brake, Input.TurnInput);
                FuelManager.ReduceFuel(Input.Accelerate);
            } else {
        ...
        // Used to switch Camera States
        void checkCameraSwitch(bool jump)
        {
            if(jump && CameraSwitchable) {
                CameraSwitched = !CameraSwitched;
                CameraSwitchable = !CameraSwitchable;
                switchCamera(CameraSwitched);
            } else if(!jump && !CameraSwitchable) {
                CameraSwitchable = !CameraSwitchable;
                switchCamera(CameraSwitched);
            }
        }

        // cam states which camera to switch to
        void switchCamera(bool cam)
        {
            SetCanMove(!cam);
        }
        ...
        void MoveVehicle(bool accelerate, bool brake, float turnInput)
        {
            ...
            float turningPower = 0; //******************************** Changed to Threshold over 3 (normally no Threshold is needed)
            if(currentSpeed > 3) {  //********************************
